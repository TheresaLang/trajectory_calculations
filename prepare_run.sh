...
if [[ ${region_def} == 'pw' ]]; then
    ...
    python ../random_starting_points/startf_from_rh.py ${exp_name} ${lagranto_run_dir} ${input_data_dir} "${first_start_time}" "${last_start_time}" ${start_time_interval} ${num_traj_per_start_time} ${num_batches} ${val_start} ${val_end} ${height_start} ${height_end}
fi 
